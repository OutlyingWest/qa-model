...
I apply LoRA adapters \citep{Hu2021LoRA} to the attention projection layers
...
parameters small while allowing task-specific adaptation. For SAQ, I also test
an extended configuration that includes \texttt{gate\_proj} to increase MLP
capacity.
...
distribution for the next token. For each choice in \{A,B,C,D\}, I evaluate
several textual variants (`` A'', ``\textbackslash nA'', ``A'') and sum
token-level log-probabilities using the cached states. The best variant score
is selected for that choice, and the highest-scoring choice is returned.

I optionally add a lightweight country-aware prior derived from the MCQ
...
For SAQ, I integrate a retrieval-augmented generation path
